...
[[nodiscard]] bool TestObj::IsAlive() const noexcept {
...
Obj::Obj(int id, std::string name)
    : id(id)
    , name(std::move(name)) {
...
void Test3() {
    ...
    }
    {
        Obj::ResetCounters();
        {
            ...
            Vector<Obj> moved_from_v(move(v));
...
void Test6() {
...

    {
        ...
        *v.begin() = 2;
        assert(v[0] == 2);
        assert(v.end() - v.begin() == static_cast<ptrdiff_t>(v.Size()));
    ...
    }
    {
        ...
        assert(all_of(v.begin(), v.end(), [](const TestObj& obj) {
            return obj.IsAlive();
        }));
    }
    {
        Vector<TestObj> v{SIZE};
        v.Insert(v.cbegin() + 2, move(v[0]));
        assert(all_of(v.begin(), v.end(), [](const TestObj& obj) {
            return obj.IsAlive();
        }));
    }
